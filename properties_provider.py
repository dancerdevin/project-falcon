...
# TODO: Incorporate whatever changes are need to work with PropertyLocationType.ZIP / LATLONG and link in to properties_analyzers.py (TBD).
...
class CompletePropertiesProvider:
  ...
  def request_properties(self, location_type: PropertyLocationType, location: str, option: PropertyGetOption) -> List[Property]:
    ...
    rentcast_provider = RentcastPropertiesProvider()
    rentcast_prop_list = rentcast_provider.request_properties(location_type=location_type, location=location, option=option)
...
class RentcastPropertiesProvider:
